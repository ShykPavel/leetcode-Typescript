...
function join(arr1: ArrayType[], arr2: ArrayType[]): ArrayType[] {
    let joinedArray: ArrayType[] = [];

    let storage = new Map<number, ArrayType>();
    ...
    for (const obj of arr2) {
        if (storage.has(obj.id)) {
            const fromArr1 = storage.get(obj.id)!;
            const merged = { ...fromArr1, ...obj};
            storage.set(obj.id, merged);
        } else { 
            storage.set(obj.id, obj);
        }
    }

    joinedArray = Array.from(storage.values()).sort((a, b) => a.id - b.id);
